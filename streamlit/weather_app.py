...
st.write("Obtén la previsión meteorológica para cualquier sitio usando Open-Meteo API.")
st.write("https://open-meteo.com/en/docs")
st.write("https://streamlit.io/: Para crear apps web interactivas")
...
fetch_button = st.button("Obtén la previsión")


# STEP6: Validamos lo que introduce el usuario
if lat < -90 or lat > 90:
    st.error("La latitud tiene que estar entre -90 y 90.")
if lon < -180 or lon > 180:
    st.error("La longitud tiene que estar entre -180 y 180.")
...
if fetch_button and (-90 <= lat <= 90) and (-180 <= lon <= 180):
    ...
    if data:
        ...
        st.write(f"Previsión para latitud: {lat}, longitud: {lon}")
